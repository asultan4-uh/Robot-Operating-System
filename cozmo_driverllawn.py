...
class CozmoRos(object):
    ...
    def _publish_objects(self):
        ...
        for obj in self._cozmo.world.visible_objects:
            ...
            self._cozmo.say_text('yay! Found it').wait_for_completed()
            self._cozmo.pickup_object(obj).wait_for_completed()
            self._cozmo.turn_in_place(degrees(-90)).wait_for_completed()
            self._cozmo.drive_straight(distance_mm(x+70), speed_mmps(100)).wait_for_completed()
            self._cozmo.place_object_on_ground_here(obj, in_parallel=False).wait_for_completed()
            self._cozmo.stop_all_motors()
            

           #for obj not in self._cozmo.world.visible_objects:
           # now = rospy.Time.now()
    ...
    def run(self, update_rate=10):
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(1,500,10):
                 self._cozmo.drive_straight(distance_mm(220), speed_mmps(100)).wait_for_completed()
                 self._publish_objects()
                 self._cozmo.turn_in_place(degrees(90)).wait_for_completed()
                 self._publish_objects()
                 self._cozmo.drive_straight(distance_mm(50), speed_mmps(100)).wait_for_completed()
                 self._publish_objects()
                 self._cozmo.turn_in_place(degrees(90)).wait_for_completed()
                 self._publish_objects()
                 self._cozmo.drive_straight(distance_mm(220), speed_mmps(100)).wait_for_completed()
