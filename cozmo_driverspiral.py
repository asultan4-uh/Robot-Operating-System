...
class CozmoRos(object):
    ...
    def _publish_objects(self):
        ...
        for obj in self._cozmo.world.visible_objects:
            ...
            self._cozmo.turn_in_place(degrees(110)).wait_for_completed()
            self._cozmo.drive_straight(distance_mm(x+280), speed_mmps(100)).wait_for_completed()
    ...
    def run(self, update_rate=10):
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(1,500,10):
                self._cozmo.drive_straight(distance_mm((i)), speed_mmps(200)).wait_for_completed()
                self._publish_objects()
                self._cozmo.turn_in_place(degrees(30)).wait_for_completed()
                self._publish_objects()
